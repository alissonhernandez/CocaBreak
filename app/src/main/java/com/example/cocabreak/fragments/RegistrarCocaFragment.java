...
public class RegistrarCocaFragment extends Fragment {
    ...
    public void onViewCreated(
        ...
        cardMini.setOnClickListener(v ->
                seleccionar(cardMini,
                        "Mini 235 ml",
                        235));

        cardLata.setOnClickListener(v ->
                seleccionar(cardLata,
                        "Lata 355 ml",
                        355));

        cardVidrio.setOnClickListener(v ->
                seleccionar(cardVidrio,
                        "Vidrio 500 ml",
                        500));

        cardBotella15.setOnClickListener(v ->
                seleccionar(cardBotella15,
                        "Botella 1.5 L",
                        1500));

        cardBotella25.setOnClickListener(v ->
                seleccionar(cardBotella25,
                        "Botella 2.5 L",
                        2500));

        cardBotella30.setOnClickListener(v ->
                seleccionar(cardBotella30,
                        "Botella 3 L",
                        3000));
    ...
    private void seleccionar(
        ...
        seleccionada.setCardElevation(
                dpToPx(6)
        );

        TextView tvTitulo =
                buscarTextViewTitulo(
                        seleccionada
                );
    ...
    private TextView buscarTextViewTitulo(
            MaterialCardView card) {

        try {

            ViewGroup contenedorHorizontal =
                    (ViewGroup) card.getChildAt(0);

            ViewGroup contenedorVertical =
                    (ViewGroup) contenedorHorizontal.getChildAt(0);

            return (TextView)
                    contenedorVertical.getChildAt(0);
